...
__global__
void blur_image(uchar* device_image, int rows, int cols, int r){
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	int idy = blockIdx.y * blockDim.y + threadIdx.y;

	if(idx + 2 * r > cols - 1 || idy + 2 * r > rows - 1) {
		return;
	}
	...
	for(int iy = 0; iy < 2 * r + 1; iy++) {
		...
		for (int ix = 0; ix < 2 * r + 1; ix++) {
			...
			int index = 3 * ((idx + ix + r) + (idy + iy + r) * cols);
	...
	int index = 3 * ((idx + 2 * r) + (idy + 2 * r) * cols);
...
int main(int argc, const char** argv){
	...
	blur_image<<< dimGrid, dimBlock >>>(device_image, originalImage.rows, originalImage.cols, 3);
